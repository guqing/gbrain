...
### v0.8（2026-04-13，BM25F title 加权 + RRF title bonus + 向量幽灵结果过滤）

**修复项**：A1（BM25F title=10x）、A2（RRF title match bonus +0.02）、A3（cosine similarity ≥ 0.25 过滤幽灵向量结果）、A5（snippet 关键词 ** 高亮标记）
...
| Q1 `redis 限流` | rank 3（统计API调用次数） | 0.333 |
| Q2 `Docker Compose PostgreSQL` | rank 1 ✅ | 1.000 |
| Q3 `Gradle 依赖冲突` | rank 1 ✅ | 1.000 |
| Q4 `git 提交邮箱修改` | **rank 1** ✅（从 rank 2 提升）| **1.000** |
| Q5 `grep 原理` | **rank 1** ✅（从 rank 3 提升）| **1.000** |
| Q6 `PostgreSQL 自然排序` | rank 1 ✅ | 1.000 |
| Q7 `抓包工具推荐` | rank 1 ✅ | 1.000 |
| Q8 `git filter-branch 邮箱` | rank 1 ✅ | 1.000 |

#### 综合评分

| 指标 | v0.6.4+bigram | **v0.8** | 及格线 | 状态 |
|------|--------------|----------|--------|------|
| MRR@10 | 0.771 | **0.917** | ≥ 0.70 | ✅ 优秀 |
| Recall@10 | 100% | **100%** | ≥ 90% | ✅ 无回退 |
...
| 编号 | 严重度 | 描述 |
|------|--------|------|
| Q1 rank=3 | 🟡 | "redis 限流" 未把 "统计API调用次数" 排首位（body-match 场景，需 Approach B reranker） |
| 冷查询延迟 | 🟠 | ~3.2s，仍超 1500ms 目标（LLM expand 是瓶颈） |
| A4 auto-embed | 🟡 | harvest 写直接写页面时不自动 embed，需用户手动 `exo embed` |
...
1. **Q1 `redis 限流` rank=3**：body-match 类查询，BM25F title boost 无法解决 → 考虑 Approach B（cross-encoder reranker）
2. **冷查询延迟**：embed API 并发超时调优，或增加更多预缓存
3. **P@5 阈值修订**：建议从 0.65 → 0.30（个人 KB 规模适配）

**目标**：v0.9 后 MRR ≥ 0.95，冷查询延迟 ≤ 1500ms。
